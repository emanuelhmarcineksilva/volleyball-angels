...
document.getElementById("novo_treino").addEventListener("click", () =>{
    window.location.href = '../View/treino_novo.html';
});

async function buscar() {
    const retorno = await fetch("../../app/Model/Treino/treino_get.php"); 
    const resposta = await retorno.json();
    if(resposta.status == "ok"){
...
async function excluir(id) {
    const retorno = await fetch("../../app/Model/Treino/treino_excluir.php?id="+id); 
    const resposta = await retorno.json();
    if(resposta.status == "ok"){
        alert(resposta.mensagem);
...
function preencherTabela(tabela){
    ...
    html += '<table>';
    document.getElementById("lista_de_treinos").innerHTML = html;  
}